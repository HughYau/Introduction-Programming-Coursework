...
void Print(vector<vector<Info> > &tables,vector<vector<Word> > &G,string &s,int l,int n){//对分词结果的打印
    int k,i,j,t;
...
void Dijkstra(vector<vector<Word> > &G,int n,int l,string &s){//在Dijkstra算法基础之上进行改进
    int k;
...
int main()
{
    ...
    while(true)
    {
        cout<<"这是基于Dijkstra算法的中文分词"<<endl<<"请输入语段（回车键结束）："<<endl;
        cin>>s;
        Wash(s);
        l = s.length()/2 + 1;//计算构造DAG的节点数
        vector<vector<Word> > G(l,vector<Word>(l));
        cout<<"请输入需要输出的路径数目："<<endl;
        cin>>n;
        G = BuildGraph(s);
        cout<<"是否启用TF-IDF优化？（1启用，0关闭）"<<endl;
        cin>>m;
        if(m)TF_IDF(s,G);
        Dijkstra(G,n,l,s);
        cout<<endl;
